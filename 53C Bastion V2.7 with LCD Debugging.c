...
void automaticLift(bool autoRaiseLift, bool autoLowerLift) {
	...
	if (autoRaiseLift || autoLowerLift){
		if (SensorValue[sensorLift] > liftEncoderMaxValue){
			autoRaiseLift = false;
			if (SensorValue[sensorLift] < liftEncoderMinValue){
				autoLowerLift = false;
			}
...
void percentLift(int percent){
	...
	if (SensorValue[sensorLift] > target){
		...
		raiseLift();
		while (SensorValue[sensorLift] < target){
			if (SensorValue[sensorLift] > 700 && toOpen)
			{
				openClawTogether();
				toOpen = false;
			}
...
void liftSensorValue()
{
	...
	displayLCDNumber(1, 5, SensorValue[sensorLift], 6);
}
